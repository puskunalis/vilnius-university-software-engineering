...
class DemoWebShopTest(unittest.TestCase):
    ...
    def run_test_with_item_list(self, item_list):
        ...
        driver.find_element(By.ID, "Password").send_keys(self.__class__.password)
        ...
        for item in item_list:
            driver.find_element(By.XPATH, f"//div[@class='item-box']//a[text()='{item}']/ancestor::div[@class='item-box']//input[@value='Add to cart']").click()
        ...
        try:
            Select(driver.find_element(By.ID, "BillingNewAddress_CountryId")).select_by_visible_text("Vatican City State (Holy See)")
        
            driver.find_element(By.ID, "BillingNewAddress_City").send_keys("Test City")
        
            driver.find_element(By.ID, "BillingNewAddress_Address1").send_keys("Test st. 123-45")
        ...
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.CLASS_NAME, "payment-method-next-step-button"))).click()
        
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.CLASS_NAME, "payment-info-next-step-button"))).click()
        
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.CLASS_NAME, "confirm-order-next-step-button"))).click()
        
        self.assertIsNotNone(driver.find_element(By.XPATH, "//strong[text()='Your order has been successfully processed!']"))
    ...
    def test_1_register_user(self):
        ...
        driver.find_element(By.ID, "Email").send_keys(self.__class__.email)
        driver.find_element(By.ID, "Password").send_keys(self.__class__.password)
        driver.find_element(By.ID, "ConfirmPassword").send_keys(self.__class__.password)
